...
SUBROUTINE vloc_psi_gamma( lda, n, m, psi, v, hpsi )
  ...
  ELSE
     ...
     v_siz_p = dfftp%nnr
#if defined(__OPENMP_GPU)
     !$omp target enter data map(to: dffts_nl, dffts_nlm)
#endif
     !
  ENDIF
  !
...
!$omp target teams distribute parallel do
  ...
  IF( use_tg ) THEN
   ...
   ELSE
     !
#if defined(__OPENMP_GPU)
     !$omp target exit data map(delete:dffts_nl,dffts_nlm)
#endif
     DEALLOCATE( dffts_nl )
...
SUBROUTINE vloc_psi_k( lda, n, m, psi, v, hpsi )
  ...
  ELSE
     ...
     !
#if defined(__OPENMP_GPU)
     !$omp target enter data map(to:igk_k,dffts_nl)
#endif
     !
  ENDIF
  !
...
!!$omp target update from(psic)
  ...
  ELSE
     !
#if defined(__OPENMP_GPU)
     !$omp target exit data map(delete:dffts_nl,igk_k) 
#endif
     DEALLOCATE(dffts_nl)
     !
  ENDIF

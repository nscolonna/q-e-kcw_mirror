...
! Written and Revised by Carlo Cavazzoni
!
! this version is using single precision for FFT
!=----------------------------------------------------------------------------------=!
...
#if defined(__INTEL_COMPILER)
#if __INTEL_COMPILER  >= 1300
!dir$ attributes align: 4096 :: emadt2, emaver, c2, c3, c2tmp, c3tmp, tg_rhos_sp, rhos_sp, ftmp, itmp
#endif
#endif
     real(DP),    allocatable :: emadt2(:)
     real(DP),    allocatable :: emaver(:)
...
!     IF( lwf ) THEN
     ...
     ELSE
        allocate( c2( c2_siz ), c3( c2_siz ) )
        allocate( tg_rhos_sp( tg_rhos_siz, nspin ) )
        allocate( rhos_sp( SIZE(rhos,1), SIZE(rhos,2) ) )
        ...
        IF( dffts%has_task_groups ) THEN
           ...
           !
           DO i = 1, nspin
              CALL tg_gather_sp( dffts, rhos_sp(:,i), tg_rhos_sp(:,i) )
           END DO
        ...
        DO i = 1, nbsp_bgrp, incr
           ...
           IF( dffts%has_task_groups ) THEN
              ...
              IF( nspin > 1 .AND. ispin_bgrp(i) /= ispin_bgrp( MIN( nbsp_bgrp, i+incr-1 ) ) ) THEN
                 ...
                 c2tmp = 0.0d0
                 c3tmp = 0.0d0
                 CALL dforce( i, bec_bgrp, vkb, c0_bgrp, c2tmp, c3tmp, tg_rhos_sp, tg_rhos_siz, itmp, ftmp, nbsp_bgrp, nspin )
                 idx_in = 1
                 DO idx = 1, incr, 2
                    IF( i + idx - 1 <= nbsp_bgrp ) THEN
                       IF( ispin_bgrp( i + idx - 1 ) == ispin_bgrp(i) ) THEN
              ...
              ELSE
                 ...
                 CALL dforce( i, bec_bgrp, vkb, c0_bgrp, c2, c3, tg_rhos_sp, tg_rhos_siz, ispin_bgrp, f_bgrp, nbsp_bgrp, nspin )
           ...
           ELSE
              ...
              CALL dforce( i, bec_bgrp, vkb, c0_bgrp, c2, c3, rhos_sp, &
                           SIZE(rhos,1), ispin_bgrp, f_bgrp, nbsp_bgrp, nspin )
              IF ( lda_plus_u ) THEN
                 c2(:) = c2(:) - vupsi(:,i)
                 c3(:) = c3(:) - vupsi(:,i+1)
     ...
     DEALLOCATE( emaver )
!
   END SUBROUTINE runcp_uspp_sp_x

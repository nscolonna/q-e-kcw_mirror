...
SUBROUTINE invfft_y_gpu( fft_kind, f, dfft, howmany )
  ...
  ELSE
     ...
     ELSE
        CALL cfft3ds_gpu( f, dfft%nr1, dfft%nr2, dfft%nr3, &
                             dfft%nr1x,dfft%nr2x,dfft%nr3x, howmany_ , 1, &
...
SUBROUTINE fwfft_y_gpu( fft_kind, f_d, dfft, howmany, stream )
  ...
  IF( dfft%lpara .and. dfft%use_pencil_decomposition ) THEN
     ...
     IF( fft_kind_int /= 3 ) THEN
        IF( howmany_ == 1 ) THEN
           CALL tg_cft3s_gpu( f_d, dfft, -1*fft_kind_int )
        ELSE
           CALL many_cft3s_gpu( f_d, dfft, -1*fft_kind_int, howmany_ )
        ENDIF
     ELSE
        CALL tg_cft3s_gpu( f_d, dfft, -1*fft_kind_int )
     ENDIF
  ...
  ELSE IF( dfft%lpara ) THEN
     ...
     IF ( fft_kind_int /= 3 ) THEN
        IF( howmany_ /= 1 ) THEN
           CALL many_cft3s_2d_gpu( f_d, dfft, -1*fft_kind_int, howmany_ )
        ELSE
           CALL tg_cft3s_2d_gpu( f_d, dfft, -1*fft_kind_int )
        ENDIF
     ENDIF

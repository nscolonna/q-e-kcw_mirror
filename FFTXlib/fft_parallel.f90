...
!
   ...
   INTERFACE tg_cft3s
      MODULE PROCEDURE tg_cft3s_dp, tg_cft3s_sp
   END INTERFACE tg_cft3s
!
CONTAINS
!
...
!
!----------------------------------------------------------------------------
SUBROUTINE tg_cft3s_dp( f, dfft, isgn )
  !----------------------------------------------------------------------------
  !
...
99 format ( 20 ('(',2f12.9,')') )
  ...
  !
END SUBROUTINE tg_cft3s_dp
!
!----------------------------------------------------------------------------
SUBROUTINE tg_cft3s_sp( f, dfft, isgn )
  !----------------------------------------------------------------------------
  !
  !! ... isgn = +-1 : parallel 3d fft for rho and for the potential
  !
  ...
  !
  TYPE (fft_type_descriptor), INTENT(in) :: dfft   ! descriptor of fft data layout
  COMPLEX(SP), INTENT(inout)             :: f( : ) ! array containing data to be transformed
  INTEGER, INTENT(in)                    :: isgn   ! fft direction (potential: +/-1, wave: +/-2, wave_tg: +/-3)
  !
  INTEGER                  :: n1, n2, n3, nx1, nx2, nx3
  INTEGER                  :: nnr_
...
!$omp end parallel do
     ...
     ! Rx, Ry, Rz
     ! clean garbage beyond the intended dimension.
     if (nsticks_x*nx1 < nnr_) f(nsticks_x*nx1+1:nnr_) = (0.0_SP,0.0_SP)
     !
  ELSE                  ! R -> G
     !
     ! Rx, Ry, Rz

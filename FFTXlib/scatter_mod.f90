...
!=----------------------------------------------------------------------=!
        ...
        INTERFACE fft_scatter_xy
           MODULE PROCEDURE fft_scatter_xy_dp, fft_scatter_xy_sp
        END INTERFACE
        INTERFACE fft_scatter_yz
           MODULE PROCEDURE fft_scatter_yz_dp, fft_scatter_yz_sp
        END INTERFACE
        INTERFACE fft_scatter_tg
           MODULE PROCEDURE fft_scatter_tg_dp, fft_scatter_tg_sp
        END INTERFACE
        INTERFACE fft_scatter_tg_opt
           MODULE PROCEDURE fft_scatter_tg_opt_dp, fft_scatter_tg_opt_sp
        END INTERFACE
...
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_xy_dp ( desc, f_in, f_aux, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
...
END SUBROUTINE fft_scatter_xy_dp
!
!-----------------------------------------------------------------------
...
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_yz_dp ( desc, f_in, f_aux, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
...
END SUBROUTINE fft_scatter_yz_dp
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_many_yz ( desc, f_in, f_aux, isgn, howmany )
  ...
  COMPLEX (DP) :: f_in(:), f_aux(:)

#if defined(__MPI)
  !
  INTEGER :: ierr, me, me2, me3, nproc3, iproc3, ncpx, nr3px, ip
  INTEGER :: i, j, it, it0, k, kfrom, kdest, offset, mc, m1, m2, i1, sendsize
...
!$omp do
        DO k = 0, howmany-1
           DO iproc3 = 1, nproc3
              DO j = 0, ncpx-1
                 IF (j>=ncp_(desc%iproc(me2,me3))) CYCLE ! control k from 0 to ncp_(desc%iproc(me2,me3))-1
...
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_tg_dp ( desc, f_in, f_aux, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
...
END SUBROUTINE fft_scatter_tg_dp
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_tg_opt_dp ( desc, f_in, f_out, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
  ! task group wavefunction redistribution
  !
...
END SUBROUTINE fft_scatter_tg_opt_dp
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_xy_sp ( desc, f_in, f_aux, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
  ! transpose of the fft xy planes across the desc%comm2 communicator
  !
  ...
  TYPE (fft_type_descriptor), INTENT(in) :: desc
  INTEGER, INTENT(in)           :: nxx_, isgn
  COMPLEX (SP), INTENT(inout)   :: f_in (nxx_), f_aux (nxx_)
...
!$omp end parallel do
     ...
     DO iproc2 = 1, nproc2
        CALL mpi_isend( f_aux( (iproc2-1)*sendsize + 1 ), sendsize, &
                        MPI_COMPLEX, iproc2-1, me2, desc%comm2, &
                        sh( iproc2 ), ierr )
     ENDDO
#endif
     !
     ...
     DO iproc2 = 1, nproc2
        ...
        !
        CALL mpi_irecv( f_in( (iproc2-1)*sendsize + 1 ), sendsize, &
                        MPI_COMPLEX, iproc2-1, MPI_ANY_TAG, &
                        desc%comm2, rh( iproc2 ), ierr )
        !IF( abs(ierr) /= 0 ) CALL fftx_error__ ('fft_scatter', ' forward receive info<>0', abs(ierr) )
     ENDDO
     ...
     !
#else
     CALL mpi_alltoall (f_aux(1), sendsize, MPI_COMPLEX, f_in(1), &
                        sendsize, MPI_COMPLEX, desc%comm2, ierr)
     !
     IF( abs(ierr) /= 0 ) CALL fftx_error__ ('fft_scatter', 'info<>0', abs(ierr) )
#endif
     !
...
!$omp end parallel do
     ...
     DO iproc2 = 1, nproc2
        IF( nproc2 > 1 ) CALL mpi_isend( f_in( ( iproc2 - 1 ) * sendsize + 1 ), &
                              sendsize, MPI_COMPLEX, iproc2-1, me2, &
                              desc%comm2, sh( iproc2 ), ierr )
     ENDDO
#endif
     IF (nproc2==1) GO TO 20
     ...
     !
#if ! defined(__NON_BLOCKING_SCATTER)
     CALL mpi_alltoall (f_in(1), sendsize, MPI_COMPLEX, f_aux(1), &
                        sendsize, MPI_COMPLEX, desc%comm2, ierr)
...
!$omp end parallel do
     ...
     IF( desc%nr2x /= desc%nr2 ) THEN
        DO k = 1, nr1p_(me2)*desc%my_nr3p
           f_in(desc%nr2x*(k-1)+desc%nr2+1:desc%nr2x*k) = (0.0_SP, 0.0_SP)
        ENDDO
     ENDIF
...
END SUBROUTINE fft_scatter_xy_sp
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_yz_sp ( desc, f_in, f_aux, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
  ! transpose of the fft yz planes across the desc%comm3 communicator
  !
...
!$omp end parallel do
     ...
     DO iproc3 = 1, nproc3
        CALL mpi_isend( f_aux( ( iproc3 - 1 ) * sendsize + 1 ), sendsize, &
                        MPI_COMPLEX, iproc3-1, me3, desc%comm3, &
                        sh( iproc3 ), ierr )
     ENDDO
#endif
     !
     ...
     DO iproc3 = 1, nproc3
        ...
        !
        CALL mpi_irecv( f_in( (iproc3-1)*sendsize + 1 ), sendsize, &
                        MPI_COMPLEX, iproc3-1, MPI_ANY_TAG, &
                        desc%comm3, rh( iproc3 ), ierr )
        !IF( abs(ierr) /= 0 ) CALL fftx_error__ ('fft_scatter', ' forward receive info<>0', abs(ierr) )
        !
        !
...
#else
     CALL mpi_alltoall (f_aux(1), sendsize, MPI_COMPLEX, f_in(1), &
                        sendsize, MPI_COMPLEX, desc%comm3, ierr)
...
!$omp end parallel do
     ...
     IF( desc%nr3x /= desc%nr3 ) THEN
        DO k = 1, ncp_(desc%mype+1)
           f_in(desc%nr3x*(k-1)+desc%nr3+1:desc%nr3x*k) = (0.0_SP, 0.0_SP)
        END DO
     END IF
...
END SUBROUTINE fft_scatter_yz_sp
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_tg_sp ( desc, f_in, f_aux, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
  ! task group wavefunction redistribution
  !
  ...
  if ( isgn > 0 ) then
     ...
     CALL MPI_ALLTOALLV( f_aux,  desc%tg_snd, desc%tg_sdsp, MPI_COMPLEX, &
                         f_in, desc%tg_rcv, desc%tg_rdsp, MPI_COMPLEX, desc%comm2, ierr)
     IF( ierr /= 0 ) CALL fftx_error__( 'fft_scatter_tg', ' alltoall error 1 ', abs(ierr) )

  else

     CALL MPI_ALLTOALLV( f_aux,  desc%tg_rcv, desc%tg_rdsp, MPI_COMPLEX, &
                         f_in, desc%tg_snd, desc%tg_sdsp, MPI_COMPLEX, desc%comm2, ierr)
     IF( ierr /= 0 ) CALL fftx_error__( 'fft_scatter_tg', ' alltoall error 2 ', abs(ierr) )
   end if
...
END SUBROUTINE fft_scatter_tg_sp
!
!-----------------------------------------------------------------------
SUBROUTINE fft_scatter_tg_opt_sp ( desc, f_in, f_out, nxx_, isgn )
  !-----------------------------------------------------------------------
  !
  ! task group wavefunction redistribution
  !
  ...
  TYPE (fft_type_descriptor), INTENT(in) :: desc
  INTEGER, INTENT(in)           :: nxx_, isgn
  COMPLEX (SP), INTENT(inout)   :: f_in (nxx_), f_out (nxx_)
  ...
  if ( isgn > 0 ) then
     ...
     CALL MPI_ALLTOALLV( f_in,  desc%tg_snd, desc%tg_sdsp, MPI_COMPLEX, &
                         f_out, desc%tg_rcv, desc%tg_rdsp, MPI_COMPLEX, desc%comm2, ierr)
     IF( ierr /= 0 ) CALL fftx_error__( 'fft_scatter_tg', ' alltoall error 1 ', abs(ierr) )

  else

     CALL MPI_ALLTOALLV( f_in,  desc%tg_rcv, desc%tg_rdsp, MPI_COMPLEX, &
                         f_out, desc%tg_snd, desc%tg_sdsp, MPI_COMPLEX, desc%comm2, ierr)
     IF( ierr /= 0 ) CALL fftx_error__( 'fft_scatter_tg', ' alltoall error 2 ', abs(ierr) )
   end if
...
END SUBROUTINE fft_scatter_tg_opt_sp
!
!----------------------------------------------------------------------------
